...
describe('DeploymentTableCell', () => {
    ...
    it('renders the build name and finish date', () => {
        render(<DeploymentTableCell columnIndex={1} tableColumn={baseColumn} tableItem={basePipeline} buildName="Build 123" />)
        expect(screen.getByRole('link', { name: /Build 123/i })).toBeInTheDocument()
        // Accept any text that looks like a relative time or a date string
        expect(screen.getByText((content) => /ago|in|second|minute|hour|day|week|month|year|2025/i.test(content))).toBeInTheDocument()
    })
